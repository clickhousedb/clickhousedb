...
boost::iterator_range<MergeTreeData::DataPartIteratorByStateAndInfo> MergeTreeData::getActivePartsCoveredBy(
    const MergeTreePartInfo & part_info,
    DataPartPtr & out_covering_part,
    std::lock_guard<std::mutex> & /* data_parts_lock */) const
{
    /// Parts contained in the part are consecutive in data_parts, intersecting the insertion place for the part itself.
    auto it_middle = data_parts_by_state_and_info.lower_bound(DataPartStateAndInfo(DataPartState::Committed, part_info));
    ...
    MutableDataPartPtr & part, SimpleIncrement * increment, Transaction * out_transaction)
{
    ...
    auto covered_range = getActivePartsCoveredBy(part_info, covering_part, lock);
    ...
    DataPartsVector covered_parts(covered_range.begin(), covered_range.end());
    ...
    else
    {
        ...
        for (auto it = covered_range.begin(); it != covered_range.end(); ++it)
        {
            (*it)->remove_time = current_time;
            modifyPartState(it, DataPartState::Outdated);
...
MergeTreeData::DataPartsVector MergeTreeData::Transaction::commit()
{
    ...
    if (!isEmpty())
    {
        ...
        {
            DataPartPtr covering_part;
            auto covered_range = data->getActivePartsCoveredBy(part->info, covering_part, data_parts_lock);
            if (covering_part)
            {
            ...
            else
            {
                ...
                removed_parts.insert(removed_parts.end(), covered_range.begin(), covered_range.end());
                for (auto it = covered_range.begin(); it != covered_range.end(); ++it)
                {
                    (*it)->remove_time = current_time;
                    data->modifyPartState(it, DataPartState::Outdated);
                    data->removePartContributionToColumnSizes(*it);
                }
            }
        }
    }
